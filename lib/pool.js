...
function getBoulderhash(block_hashing_blob, callback){
    apiInterfaces.rpcDaemon('getboulderhash', {block_hashing_blob_hex: block_hashing_blob.toString('hex'), version: 2}, callback);
}
...
function processShare(miner, job, blockTemplate, nonce, resultHash, callback){
    ...
    function finishProcessing(shareType, strHash)
    {
        if (strHash !== resultHash) {
            log('warn', logSystem, 'Bad hash from miner %s@%s', [miner.login, miner.ip]);
            log('warn', logSystem, 'miner said:  %s', [resultHash]);
            log('warn', logSystem, 'daemon said: %s', [strHash]);
            callback(true, false);
            return;
        }
      
        var hash = new Buffer(strHash, 'hex');
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        finishProcessing('trusted', resultHash);
    }
    else {
        convertedBlob = cnUtil.convert_blob(shareBuffer);
        getBoulderhash(convertedBlob, function (error, result) {
            if (error)
            {
                log('error', logSystem, 'Error calling getboulderhash: %j', [error]);
                callback(false, false);
                return;
            }
            finishProcessing('valid', result.block_hash_hex);
        });
    }
}
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        ...
        case 'submit':
            ...
            processShare(miner, job, blockTemplate, params.nonce, params.result, function (succeeded, shareAccepted) {
                if (!succeeded){
                    log('warn', logSystem, 'Pool could not process a share');
                    sendReply('Pool failed');
                    return;
                }
    ...
    async.each(config.poolServer.ports, function(portData, cback){
        var handleMessage = function(socket, jsonData, pushMessage){
            ...
            var sendReply = function(error, result){
                if (error) log('info', logSystem, 'Sending back error: ' + error);
            ...
            socket.on('data', function(d){
                ...
                if (dataBuffer.indexOf('\n') !== -1){
                    ...
                    for (var i = 0; i < messages.length; i++){
                        ...
                        log('info', logSystem, 'got message: ' + message);
                        var jsonData;
                        try{
                            jsonData = JSON.parse(message);
                        }
                        catch(e){
                            log('info', logSystem, "THIS IS NOT JSON");
